...
//
// Drawing in, drawing out: this takes the file's lines, rearranges them and
// hands them back, so the same code runs behind the command line and in the
// browser. build-details.js wraps it in argument parsing and file IO.
//
...
/**
 * Arranges one drawing. `lines` is the file split on its line endings, as
 * dxf-io's splitLines returns it, and is edited in place and handed back.
 *
 * The flags are the command line's, named the same way: `storey` narrows to
 * one storey, `first`/`last` to a range of beam numbers within each storey,
 * and `replaceSpanLabels` removes Prota's own span labels from the plan.
 * `srcName` and `outName` only ever appear in the report.
 *
 * Everything it has to say goes to `log`, one call per line, because the
 * report is the only account of what the run decided -- see readReport in the
 * web front end, which reads its numbers back out of this text.
 */
function buildDetailSheet(options) {
  const { lines, usesCRLF } = options;
  const srcName = options.srcName || "drawing.dxf";
  const outName = options.outName || srcName;
  const log = options.log || (() => {});
  // Kept under the flag names the command line uses, so the body below reads
  // the same as it did when these were module-level constants.
  const STOREY_FLAG = options.storey ?? null;
  const FIRST = options.first ?? 1;
  const LAST = options.last ?? Infinity;
  const REPLACE_SPAN_LABELS = Boolean(options.replaceSpanLabels);
  ...
  log(`${srcName}: ${labels.length} span labels -> ${beams.length} beams.`);
  ...
  for (const [layer, list] of strays) {
    ...
    log(`  ** ${String(list.length).padStart(3)} on "${layer}" (beams ${beams.join(", ")})`);
    for (const s of list.slice(0, 4))
      log(`       ${s.e.type} x[${s.e.x0.toFixed(0)}..${s.e.x1.toFixed(0)}] y[${s.e.y0.toFixed(0)}..${s.e.y1.toFixed(0)}]${s.e.text ? " " + JSON.stringify(s.e.text.trim()) : ""}`);
  }
  ...
  log(`  ${dropped} entit(ies) deleted from ${[...DROP_LAYERS].join(", ")}.`);
  log(
  ...
  if (layerRes.created) log(`  created layer "${frameLayer}"`);
  log(`  ${ctx.movedBlocks.size} dimension blocks moved with their dimensions.`);
